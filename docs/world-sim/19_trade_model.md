...
- Дороги на изображении карты не распознаются автоматически: pathfinding/image recognition в этом режиме не используются.
- Для точного совпадения с дорогой пользователь вручную включает режим **«Режим разметки маршрутов»** и кликами по карте задаёт `TradeRoute.Points`.
- Точки сохраняются в нормализованных координатах `0..1` и образуют authored polyline маршрута.
- Полилиния сразу отображается на карте и может быть скорректирована через undo/clear.
- Чтобы маршрут визуально совпадал с дорогой, точки стоит ставить на поворотах, развилках и заметных изгибах.

## Exporting authored points

- После разметки пользователь нажимает **«Скопировать Points»**.
- Приложение копирует в буфер обмена `RouteId` и C# initializer для `Points`.
