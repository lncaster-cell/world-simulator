...
- Маршрут выбирается через поселение-источник **«Откуда»** и список назначений (до 8 пунктов), а затем активное направление.
- Старый route-id-first UX не используется как основной пользовательский сценарий.
...
- Каждое выбранное назначение формирует отдельный `TradeRoute` (A→B, A→C, A→D — разные маршруты).
