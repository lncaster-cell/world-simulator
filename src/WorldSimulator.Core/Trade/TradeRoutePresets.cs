...
public static class TradeRoutePresets
{
    ...
    {
        points = [];
        if (!RiviaSettlementPresets.TryGetLocation(route.FromSettlementId, out var start))
        {
            return false;
        }

        if (!RiviaSettlementPresets.TryGetLocation(route.ToSettlementId, out var end))
        {
            return false;
        }
