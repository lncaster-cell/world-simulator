...
public sealed class RoutePathExtractor
{
    ...
    {
        var sx = (int)Math.Round(pt.X * (width - 1));
        var sy = (int)Math.Round(pt.Y * (height - 1));
    ...
    static List<(int X, int Y)> FindPath(bool[] mask, int w, int h, (int X, int Y) s, (int X, int Y) t)
    {
        ...
        while (q.Count > 0)
        {
            ...
            foreach (var dy in d)
            {
                ...
                var ni = (ny * w) + nx;
                if (!mask[ni] || !seen.Add(ni)) continue;
                prev[ni] = cur;
                q.Enqueue(ni);
            }
        }
    ...
    static List<string> SplitCsv(string line)
    {
        ...
        foreach (var ch in line)
        {
            ...
            if (ch == ',' && !quoted) { result.Add(cur); cur = string.Empty; }
            else cur += ch;
        }
    ...
    static string NormalizeName(string name) => name.ToLowerInvariant().Replace('ö', 'o').Replace('-', '_').Replace(' ', '_') switch
    {
        "tokrus" => "thokur_rus",
        var n => n
    };

    static readonly Dictionary<string, string> NodeMap = new(StringComparer.OrdinalIgnoreCase)
    {
        ["N_HIGHROCK"] = "highrock", ["N_MLYNEK"] = "mlynek", ["N_WARDMARK"] = "wardmark", ["N_RIVENSTAL"] = "rivenstal", ["N_GAVERN"] = "gavern", ["N_BRNO"] = "brno", ["N_WODENZ"] = "wodenz", ["N_GOTHA"] = "gotha", ["N_TOKRUS"] = "thokur_rus"
    };

    static string BuildTradeRouteId(string from, string to, string type)
    {
